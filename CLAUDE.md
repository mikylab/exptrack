...
- **`experiments`** — run metadata (id, name, status, created_at, duration_s, git_branch, git_commit, git_diff, hostname, python_ver, notes, tags, output_dir, **session_node_id** — nullable FK into `session_nodes` set only by `%exptrack promote`, **deleted_at** — nullable ISO timestamp; non-null = soft-deleted/Trash. Indexed on `deleted_at`)

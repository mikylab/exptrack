...
- **Say which run a run should be compared against** — `exptrack variant-of <id> <baseline>` (and `POST /api/experiment/<id>/set-variant-of`) declares that one run is a variant of another, and that target then overrides chronology in both the "vs previous" strip and the "What changed" card. Chronology is the wrong baseline for the commonest loop there is: re-run one notebook with a different model and "the previous run of this script" is whatever happened to run last, not the run you mean to compare against. Omit the baseline to clear the link. A stale link — the target trashed, or still running — falls back to the chronological pick rather than leaving the run with no comparison at all.

### Security
- **The dashboard auth token is no longer accepted in the query string on every route** — it is genuinely needed there only for `/api/file/`, because an `<img>` tag cannot send an `Authorization` header. Everywhere else a query token widened the exposure surface (URLs land in logs, proxies and history) and softened the CSRF story for no benefit; those routes now require the header.
- **Renaming a run can no longer move its output folder out of the project** — the new name was used to build the destination path with no sanitization, so a name like `../../evil` relocated the run's output directory outside `outputs/` and outside the project root. The destination is now reduced to a single path component and verified to sit under `outputs/`.

### Fixed
- **`--resume` could silently continue a run you had deleted** — both "latest run of this script" lookups (`exptrack run --resume`, `run-start --resume latest`) matched on script alone. With `auto_trash_failed` on, the likely sequence — the previous attempt failed, was auto-trashed, and you reran with `--resume` — appended an entire run's metrics, artifacts and timeline to a row hidden from every list, stat and comparison, with no error and no new run appearing. Both lookups now skip trashed runs, and `Experiment.resume` refuses one outright and points at Restore. Both also tie-break on insertion order, so two runs launched inside the same clock tick resolve to the genuinely-latest one.
- **A failed schema migration marked the database as migrated anyway** — the `_migrate_*` helpers swallow errors, and the version stamp was written regardless. One transient `database is locked` during an `ALTER` therefore left a column missing while the database was stamped current — and because a matching stamp skips the migration entirely, no later connection ever retried it. The stamp is now written only when every helper succeeded, so the next open picks up where it left off.
- **A sweep's plots could be moved to Trash with the wrapper run** — under `exptrack run`, savefig and auto-output capture stay pointed at the wrapper for the whole script, so a sweep that plots per iteration without logging a metric leaves its only artifact rows there. The wrapper was then trashed for "having no metrics", taking those rows with it. It now counts artifacts too, ignoring the rows every wrapper gets for free (the output directory and the captured stdout/stderr logs).
- **Negative numbers were dropped from captured params** — `--lr -0.5` was stored as `lr: true` with the value discarded, because any `-`-prefixed following token was read as the next flag. For a script that doesn't use argparse — exactly what raw-argv capture exists for — nothing corrected it afterwards, so the experiment table, the run name and every diff showed the wrong value.
- **NaN and infinity could be written to the metrics table from the pipeline commands** — Python's JSON round-trips them, so a training script writing `NaN` to `results.json` was ingested by `run-finish --metrics` without complaint and stored as a metric row with no value at all. Every pipeline path (`run-finish`, `log-metric`, `log-result`, `create`) and the dashboard's manual metric entry now refuse non-finite values, and a non-numeric value is reported per key instead of skipped in silence.
- **`create --date` accepted anything** — a value that wasn't a timestamp crashed `run-finish` *before* it marked the run done, leaving the run stuck `running` and its metrics file never ingested. It is validated when the run is created.
- **Renaming a run from the experiment table could corrupt its name** — the editor was seeded from the text as displayed, which is middle-truncated for a long name, so editing anything committed a name containing a literal `…` with its middle missing. This hit precisely the runs the "Needs naming" filter sends you there to rename. Relatedly, a background list refresh while you were mid-rename detached the input and its blur handler committed the half-typed value; the in-progress edit now survives that refresh.
- **Compare's overlay charts drew two runs as if their points lined up** — both series were plotted by position, so runs with different logging cadence or different downsampling had point *N* of one drawn above point *N* of the other, against the first run's step labels. They are now aligned on their real x-values, which is the entire question the view exists to answer.
- **Clicking a chart point to delete it could delete a different point** — on a series logged without steps and long enough to be downsampled for display, the position in the drawn line is not the position in the database, so the confirmation named one point and the deletion removed another. That case is now refused with an explanation.
- **A single failed request left Compare and Timeline blank with no message** — several follow-up fetches read straight through a null response and threw mid-render, so the panel simply never drew, which is the silent-blank failure the rest of the dashboard is careful to avoid. They now render the same error state the Images and Data Files tabs already did.
- **A run that failed with a message but no traceback showed no "Run failed" panel**, surfacing the error only as a row in the params table. The manual-experiment modal also stored numeric params as strings, unlike the "+ Add Param" form beside it.
- **Session Trees: re-running a notebook no longer duplicates or misplaces recorded work** — Run-All is the way a session notebook gets re-executed, and four paths through the magics failed to recognize the replay, each growing the tree on every pass. (1) Cells above your first checkpoint — imports, data loading — were recorded onto whatever node the *previous* pass ended on, appending a copy each time while the root's own copies sat untouched; `%exptrack session start` on an already-active session now returns to the session root, which is what re-executing the notebook's first magic means. (2) Re-declaring the checkpoint you are already standing on didn't re-arm the replay cursor, so every cell of that checkpoint was appended again per pass. (3) `%%setup` cells deduplicated only against the most recently recorded one, so a node with two or more prep cells doubled its setup store on every Run-All — until the byte budget started evicting real content, and each duplicate replayed as its own timeline event when the node was promoted. The setup store now carries its own replay cursor, like tracked cells. (4) The branch-label collision guard compared only against the *original* node, so re-running an edited branch forked `A (2)`, then `A (3)`, then `A (4)` — one per pass; it now recognizes the fork a previous pass already created — by its first recorded cell, so renaming the fork (which the notice inviting you to rename it made likely) or promoting it to a checkpoint doesn't hide it — while genuinely distinct ideas still each get their own node.
- **Session Trees: deleting a node no longer copies its cells onto the surviving node** — the live session cached the current node's cells and its replay cursors, and deleting the node you were working on re-pointed the session by assignment, leaving all of that describing the *deleted* node. The next cell you ran was written as "the deleted branch's cells, plus this one" onto the checkpoint the deletion fell back to — so a trashed branch's code silently reappeared on its parent, and restoring the branch then duplicated it. Every path that changes the current node now goes through the one function that refreshes the cache.
- **Ending a session from the dashboard behaves like `%exptrack session end`** — the route carried its own copy of the abandon-open-branches update without the trashed-row filters, so it relabelled branches sitting in the Trash (which then came back as `abandoned` on restore) and treated trashed children as children, leaving a branch whose only children are trashed un-abandoned forever. Both paths now share one implementation, and ending the session that is live in the current process also captures its open branch's working-tree diff first, as the magic always did.
- **Watching a live run finish no longer breaks the detail view** — the moment a run being watched transitioned out of `running`, the auto-refresh poll stopped itself *before* reading the run id it was about to refresh, so the detail panel was replaced by an "Experiment not found" error card whose Retry button looped on the same error. The id is now captured before the poll stops, and the finished run refreshes in place.
- **`with Experiment()` blocks that call `finish()` explicitly no longer crash at exit** — the context manager called `finish()` unconditionally on the way out, and a second `finish()` raises. A clean block that finished itself died with `RuntimeError: Cannot finish twice`, and if the block raised *after* an explicit finish, that RuntimeError replaced the user's real exception as the one propagating. Both exit branches now check whether the run is already finished, matching every other finish path.
- **A run dropped without `finish()` no longer loses its last metrics or crashes the next run** — metric writes coalesce into a commit window, and the flush registry held runs by weak reference: a run object garbage-collected mid-window (function scope, an exception unwind) became unreachable by every flush, its buffered points sat in the shared connection's open transaction, and the next `Experiment()` died on `BEGIN IMMEDIATE` — destroying the earlier run's points in the rollback. The pending state is now tracked on the connection (per thread), so the flush lands those rows no matter what happened to the run object, and `close_db()` commits before closing instead of silently rolling a pending window back.
- **The storage report no longer sizes deduplicated git diffs at zero** — a column-name typo in the shared-diff sizing query (`hash` for `diff_hash`) was silently swallowed, so every content-addressed diff body — the normal case for all new runs — contributed 0 bytes to "Largest runs" and the Trash figures, mis-ranking any run whose bulk is a big working-tree diff.
- **The dashboard's metric prune deletes exactly the set its confirm described** — the confirmed request re-ran target selection from scratch, so points logged while the dialog was open (a training run is usually still going) were deleted beyond what the confirm quoted. The dry-run now hands back a preview token and the confirmed delete removes precisely the previewed set — refusing, rather than guessing, if the preview has expired. The prune also truncates the WAL afterwards like the other destructive routes, so pruning to reclaim space no longer leaves a ballooned `-wal` file next to the database.
- **Exact per-table sizing now works on older SQLite builds** — the dbstat query used the `sqlite_schema` alias (SQLite ≥ 3.33), so a stock Python 3.9 on Ubuntu 20.04 silently degraded every "exact" figure to an estimate while claiming dbstat was unavailable. It now uses `sqlite_master`, which every version has.
- **The permanent-delete confirm sizes every output directory the delete removes** — when a run's recorded `output_dir` and its name-derived `outputs/<name>` folder had diverged (a changed `outputs_dir` config, a failed rename), the preview stopped at the first one while the delete trashed both, so the confirm under-stated what was about to move to the Trash.
- **A hand-edited `metric_keep_every: 1e999` no longer crashes every `log_metric`** — JSON happily parses `1e999` as infinity, and converting that to an int raises an error the thinning guard didn't catch, so a config value that should degrade to "keep everything" instead raised out of the user's training loop. Settings-panel saves of such a value now get a friendly error instead of a 500 as well.
- **A huge `?offset=` on the experiment list is clamped instead of a 500** — query-param integers beyond SQLite's 64-bit range overflowed the parameter binding; they're now clamped alongside the existing `limit` cap.
- **Long fully-thinned stretches can't leave a stored point invisible** — with thinning on, a kept metric point followed by hundreds of dropped ones never re-ticked the commit window, so the point stayed uncommitted (and invisible to the dashboard) until something else flushed. Dropped points now tick the window too.
- **Step-less metric exports are deterministic** — a series logged without steps was exported in whatever order SQLite returned, so the summary's `first`/`last` (the headline numbers of the default export) could be any point in the series. Insertion order now breaks the tie, as it already did everywhere else.
- **`exptrack storage` no longer walks the database three times** — the Trash and orphaned-rows sections each recomputed the whole-file page scan the report had already done once; they now reuse it. The orphan note also no longer claims snapshot/diff blob orphans are swept automatically on CLI exit (they never were — only `exptrack clean` removes those).
- **Metric thinning could silently record nothing** — `metric_keep_every: N` was applied as "keep points whose *step* divides by N", which quietly assumes you log on every step. The common `if (i + 1) % 5 == 0:` pattern logs steps 5, 10, 15 …, and with `metric_keep_every: 999` the two are coprime, so no step ever qualified and the run stored **zero** metrics — a setting that reads as "thin this a bit" acting as "discard everything", with an empty chart as the only feedback. Even a friendly factor was wrong: `1000` on a 200-step run kept step 0 and nothing else. Thinning now counts **logged points per metric key** — 1 of every N, always keeping a key's first point — which is what the setting has always claimed to do, works at any logging cadence, and can never take a series to zero. A step-less `log_metric(key, value)` series is now thinned on the same rule instead of being exempt from the setting entirely, and an unusable value in a hand-edited config degrades to keeping everything rather than dropping it.

  Thinning also no longer does this silently: the first point a run drops prints a one-line notice naming the factor and how to turn it off. In the dashboard the field was **"Save every Nth step"**, sitting directly under "Chart max points: 500" — which invites reading it as a point budget when it is the opposite. It's now **"Store 1 of every N points"**, and any value above 1 shows what it means for a real run ("a run logging 1,000 points will store about 200 — dropped points cannot be recovered") with a pointer to `exptrack prune`, which thins what you already recorded and always keeps each series' first, last, min and max.

  If a run of yours recorded no metrics, check `metric_keep_every` in `.exptrack/config.json`; set it back to `1`. Points already dropped were never written and can't be recovered.

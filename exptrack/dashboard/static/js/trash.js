...
function _renderTrashSessionSection(rows) {
  ...
  html += '<table class="trash-table">' +
    ...
    '</tr></thead><tbody>';
  for (const r of rows) {
    const nm = escJsAttr(r.name || '(unnamed session)');
    html += '<tr>' +
      '<td>' +
...
// Callable from two places: the Trash view's Sessions section, and the
// "↺ Restore session" button the session view shows for a trashed session
// (reachable via an experiment's back-link). Only refresh the views that are
// actually mounted.
async function restoreSession(id) {
  const r = await postApi('/api/session/' + encodeURIComponent(id) + '/restore', {});
  if (r && r.ok) {
    owlSay('Session restored');
    if (document.body.classList.contains('trash-active')) loadTrashList();
    if (typeof loadSessionsList === 'function') loadSessionsList();
    // Re-render the open session so its header drops the "in Trash" tag.
    if (typeof selectSession === 'function' &&
        typeof _activeSessionId !== 'undefined' && _activeSessionId === id) {
      selectSession(id);
    }
  } else {
  ...
  for (const r of rows) {
    ...
    html += '<tr>' +
      ...
      '<td><div class="trash-row-actions">' +
        ...
        '<button class="dc-button danger" onclick="trashPermanent(\'' + r.id + '\',\'' + escJsAttr(r.name || '') + '\')">Delete…</button>' +
      '</div></td>' +
    '</tr>';
  }
  html += '</tbody></table></div>';
...
function _renderTrashNodeSection(sessions, nodeTotal) {
  ...
  for (const g of sessions) {
    ...
    // The session itself may be in the Trash (and so also listed in the
    // Sessions section above). Restoring one of its nodes then restores
    // nothing the user can see, because the session stays hidden from every
    // live list — say so and point at the session's own Restore.
    const sessDeleted = !!g.session.session_deleted;
    html += '<div class="trash-session-group' + (sessDeleted ? ' session-trashed' : '') + '">' +
      '<div class="trash-session-head">' +
        ...
        (sessDeleted ? '<span class="trash-session-note">session is in Trash — ' +
          '<a href="#" onclick="restoreSession(\'' + escJsAttr(sid) + '\');return false">restore the session</a>' +
          ' to see these again</span>' : '') +
        '<button class="dc-button danger" onclick="emptyTrashSession(\'' + sid + '\',\'' +
          escJsAttr(sname) + '\')">Empty (' + nodes.length + ')</button>' +
      '</div>' +
      ...
      html += '<div class="trash-row">' +
        ...
        '<div class="trash-row-actions">' +
          ...
          '<button class="trash-purge-btn" onclick="purgeTrashNode(\'' + sid + '\',\'' + n.id + '\',\'' +
            escJsAttr(n.label || '') + '\')">Delete forever</button>' +
        '</div>' +
      '</div>';
    }
    html += '</div></div>';
